...
const prodMod = require("../model/prodmodel")
const initializeDatabase = async (req, res) => {
    try {
        const response = await axios.get('https://s3.amazonaws.com/roxiler.com/product_transaction.json')
        const products = response.data

        await prodMod.deleteMany()
        await prodMod.insertMany(products)
...
const getTransactions = async (req, res) => {
    try {
        ...
        const products = await prodMod.find(query)
            .skip((currentPage - 1) * itemsPerPage)
            .limit(itemsPerPage)

        const totalProducts = await prodMod.countDocuments(query)
...
const getSalesStatistics = async (req, res) => {
    try {
        ...
        const soldItems = await prodMod.find({
            sold: true,
            $expr: {
        ...
        const unSoldItems = await prodMod.find({
            sold: false,
            $expr: {
                $eq: [{ $month: "$dateOfSale" }, parseInt(month)]
            }
...
const getBarChartData = async (req, res) => {
    try {
        ...
        const barChartData = await prodMod.aggregate([
            {
                $addFields: {
...
const getPieChartData = async (req, res) => {
    try {
        ...
        const pieChartData = await prodMod.aggregate([
            {
                $addFields: {
                    soldMonth: { $month: "$dateOfSale" }
                }
